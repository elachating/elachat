...
public class UpdateManager {
    ...
    public UpdateManager(JSONArray args, CallbackContext callbackContext, Context context) {
        this(args, callbackContext, context, "http://127.0.0.1:8080/update_apk/version.xml");
    }
    ...
    private void compareVersions() {
        ...
        String versionContents = version.getUcontents();
        //比对版本号
        //检查软件是否有更新版本
        if (versionCodeLocal != versionCodeRemote) {
            if (isDownloading) {
                ...
                // 显示提示对话框
                msgBox.showNoticeDialog(noticeDialogOnClick,oncancelDialogOnClick,versionContents.replaceAll ( "\n", "\\\\"+System.getProperty("line.separator")));
    ...
    private OnClickListener oncancelDialogOnClick = new OnClickListener() {
        @Override
        public void onClick(DialogInterface dialog, int which) {
            dialog.dismiss();
           // mHandler.sendEmptyMessage(Constants.DOWNLOAD_CLICK_START);
