...
public class CheckUpdateThread implements Runnable {
    ...
    public void run() {
        ...
        String ucontents = getUpdatecontents();
        queue.clear(); //ensure the queue is empty
        queue.add(new Version(versionCodeLocal, versionCodeRemote,ucontents));
     ...
     * @return
     */
    private String getUpdatecontents() {
        ...
        if (null != getMHashMap()) {
            updatecontents = String.valueOf(getMHashMap().get("updatecontents"));
        }
